...
しかし、Rustではあるメモリに対して、不変な参照 `&'a T` がたくさんあるか、可変な参照 `&'a mut T` が1つあるか、どちらか一方であることが保証されています。この性質（Shared XOR mutabilityとしてよく知られています）は、シングルスレッドのプログラムでも多くのバグを防ぐのに役立ちますが、マルチスレッドのプログラミングでは非常に強力な性質となります。複数のスレッドに参照が分散している場合でも、読み取り専用のスレッドが多数存在するか、単一のスレッドだけが書き込みを行い、他のスレッドは読み取りだけを行う場合、メモリの安全性が保証されるためです。
